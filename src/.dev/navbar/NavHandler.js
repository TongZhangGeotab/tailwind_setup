class NavHandler {
    ...
    */
    generateContent(){
        ...
        let centerPane            = document.getElementById('installTailwind');
    ...
    enableDisplayToggle(){
        ...
        displayToggle.addEventListener('click', () => {
            if(this.focus){
                ...
                global.geotab.addin.installTailwind.blur();
            } else {
                displayToggle.innerHTML = 'Blur add-in';
                global.geotab.addin.installTailwind.focus(global.api, global.state);
            }
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'installTailwind'){
                    geotab.addin.installTailwind.blur();
                } else {
                    geotab.addin.installTailwind.focus(global.api, global.state);
                }
            }
        }, 100);
    }
